...
public class ProductColorsCustomDetail implements UserType {
    @Override
    public int[] sqlTypes() {
        ...
        try {
            ...
            if(value.isPresent()) {
                ...
                response = objects.stream().map(color ->
                        new ProductColorDetails((String) color.get("id"),
                                (String) color.get("name"),
                                (Double) color.get("quantity"),
                                (Boolean) color.get("hasStoke"),
                                (String) color.get("rgb"))).collect(Collectors.toList());
            }
        }
        catch (Exception e){
